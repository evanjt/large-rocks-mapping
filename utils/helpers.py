...
def convert_annotation_to_yolo(
    annotation, patch_size=640, bbox_width=16, bbox_height=16
):
    """
...
def create_yolo_annotation_files(
    dataset, output_dir, patch_size=640, bbox_width=32, bbox_height=32
):
    """
        ...
        with open(txt_file_path, "w") as f:
            ...
            for annotation in sample.get("rocks_annotations", []):
                yolo_line = convert_annotation_to_yolo(
                    annotation, patch_size, bbox_width, bbox_height
                )
                f.write(yolo_line + "\n")


def prepare_yolo_training_files(
    dataset,
    root_dest_dir,
    split,
    patch_size=640,
...
def prepare_yolo_training_files_all_splits(
    train_samples,
    val_samples,
    test_samples,
    patch_size=640,
    ...
    prepare_yolo_training_files(
        val_samples, root_dest_dir, "val", patch_size, bbox_width, bbox_height
    )
    prepare_yolo_training_files(
        test_samples,
        root_dest_dir,
    ...
    print(
        "----------- YOLO data preparation complete for all splits. ------------"
    )
...
def split_without_overlap(dataset, split=(0.8, 0.1, 0.1), seed=None):
    ...
    subgroups = overlap[
        "overlap_groups"
    ]  # there is only overlapping groups with each 16 images
...
def save_metrics(results, output_dir, file_name):
    ...
    output_file_path = os.path.join(
        output_dir, "metrics_results_" + file_name + ".csv"
    )
    # save to file
    with open(output_file_path, "w") as f:
        f.write("precision, recall, f1, f2, map50, map5095\n")
        f.write(
            f"{precision:.4f}, {recall:.4f}, {f1:.4f}, {f2:.4f}, {map50:.4f}, {map5095:.4f}\n"
        )
...
def validate_model(
    ...
    cm = ConfusionMatrix(
        nc=model.nc, conf=conf_thres, iou_thres=iou_thres, task="detect"
    )
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # inference + NMS
            preds = non_max_suppression(model(imgs), conf_thres, iou_thres)
            ...
            for si, dets in enumerate(preds):
                ...
                gt_boxes_xyxy = xywhn2xyxy(
                    true_bb[gt_mask], imgs.shape[3], imgs.shape[2]
                )
                ...
                cm.process_batch(dets_t, gt_boxes_xyxy, gt_cls)
    ...
    f2 = (
        ...
        * precision
        * recall
        / (2.0**2 * precision + recall + 1e-16)
    )

    if verbose:
        print(
            f"Precision: {precision:.3f}, Recall: {recall:.3f}, F1: {f1:.3f}, F2: {f2:.3f}"
        )
